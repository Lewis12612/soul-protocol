...
> **版本**: V3.8.8-beta4  
> **生产就绪**: 自评 9.2/10（功能实现版本，未完整测试）（内部验证，外部审计待完成）
...
| Provider | 状态 | 备注 |
|----------|:----:|------|
| DeepSeek V4 (API) | ✅ 已验证 | spawn 超时正常，日终执行稳定 |
| 讯飞星辰 GLM-5.1 | ⚠️ 部分兼容 | spawn 可能超时（长思考链），建议调高 timeoutSeconds ≥ 300 |
| Claude (Opus/Sonnet) | ⚠️ 未验证 | 理论兼容，需实测 |
| Gemini | ⚠️ 未验证 | 理论兼容，需实测 |
| Qwen3.5+ | ⚠️ 未验证 | 理论兼容，需实测 |

**最低 OpenClaw 版本**: 2026.3.x+
**最低 Node.js**: ≥ 22
...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| **2026-06-21** | **V3.8.8-beta4** | **四层冗余全覆盖（A+B+C+D × 6协议）+ SESSION-STATE工作记忆 + 协议依赖链 + L2格式校验 + 统一验证接口** |
| **2026-06-21** | **V3.8.8-beta3** | **P0 生产缺陷修复（eod-pending假完成+注入≠执行+模板路径错误）+ 全量路径参数化 + 双实例交叉审计** |
| **2026-06-20** | **V3.8.8-beta2** | **三角因子指令解析 + execute_protocol tool + L2协议标记 + 工作流文档 + 合并去重 + 开源打包 + GitHub上线** |
...
| 阶段 | 内容 | 预估 |
|------|------|:--:|
| V3.8.8-beta4 | 三角因子指令解析 + execute_protocol tool + 合并去重 | 2-3天 |
...
*创建: 2026-05-02*  
*最后更新: 2026-06-20 11:21（V3.8.8-beta4：三角因子指令解析 + execute_protocol tool + L2标记 + 工作流文档）*  
*版本: V3.8.8-beta4*